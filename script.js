...
// OPPOSITES — Unit 6 (word, correct opposite, distractors)
const OPPOSITES = [
...
// WORD FORMS — Unit 4 (sentence with blank, accepted answers, hint)
const FORMS = [
...
function buildFlashcards(stage){
    ...
    $("#fcKnow").addEventListener("click",()=>{knew++;addPoints(2);nextOrFinish();});
    $("#fcReview").addEventListener("click",()=>{nextOrFinish();});
...
function buildMatch(stage){
    ...
    $$(".chip").forEach(chip=>{
        ...
        // touch / click fallback: tap word then tap a drop
...
/* =====================================================================
   MODE 3 — OPPOSITES (tap the opposite)
===================================================================== */
    ...
    function render(){
        ...
        $("#oHolder").innerHTML=`
            <div class="q-card">
                ...
                <div class="opt-row">
                    ${shuffle(q.opts).map(o=>`<button class="opt" data-o="${o}">${o}</button>`).join("")}
                </div>
            </div>`;
...
/* =====================================================================
   MODE 4 — WORD FORMS (type the answer)
===================================================================== */
function buildForms(stage){
    stage.innerHTML=`<p class="instructions">Type the correct <strong>form of the word</strong> in each blank. Use the hint!</p>
        ...
        <button class="check-forms-btn" id="checkForms">Check my answers ✓</button>
    ...
    $("#checkForms").addEventListener("click",()=>{
        ...
        FORMS.forEach((f,k)=>{
            ...
            if(f.answers.includes(val)){inp.classList.add("correct");correct++;}
        ...
        if(correct===FORMS.length){
            burst();
            $("#checkForms").outerHTML=`<button class="stage-done-btn" onclick="backToHub()">Back to menu →</button>`;
        }
        markDone("forms");
    });
}

// used by inline onclick in stage banners
